...
async def get_courses(title: str = Query(None, title="Título do curso", description="Filtro por título", min_length=3), db: AsyncSession = Depends(get_session)) -> Union[List[CourseModel], dict[int, CourseModel]]:
...
async def get_course(course_id: int = Path(..., title="ID do curso", description="Deve ser maior ou igual a 1", ge=1), db: AsyncSession = Depends(get_session)) -> CourseModel:
    async with db as session:
...
async def create_course(course: CourseModel, db: AsyncSession = Depends(get_session)) -> CourseModel:
...
async def update_course(course_id: int, course: CourseModel, db: AsyncSession = Depends(get_session)) -> CourseModel:
    async with db as session:
        query = select(CourseModel).where(CourseModel.id == course_id)
        result = await session.execute(query)
